...
from numpy import std, mean, array, ones, random
...
def select(pl, scores, notquite=False): ## assumes a player array and a scores array with the same length
    ...
    for p in pl:
        for t in types:
            if t[0] == p.__name__ and t[1] == p.init_values():
                break
        else:
            types.append((p.__name__, p.init_values()))
    
    for i in range(len(types)):
        sc_by_type.append([])
        for j in range(len(pl)):
            if types[i][0] == pl[j].__name__ and types[i][1] == pl[j].init_values():
